...
<div class="flex justify-start w-full">
    <div class="flex flex-col py-3 px-2 rounded-t-lg rounded-r-lg border-black border-2 bg-pinktt w-fit max-w-96">
        @if($msg->endereco_anexo)
        <div class="w-full flex flex-col justify-center items-center">
            <img class="border border-black rounded-md max-w-64" src="{{ asset($msg->endereco_anexo) }}" alt="">
        </div>
        @endif
        <pre class="w-full flex items-center whitespace-pre-wrap text-wrap break-all">{{ $msg->conteudo_mensagem }}</pre>
    </div>
</div>
